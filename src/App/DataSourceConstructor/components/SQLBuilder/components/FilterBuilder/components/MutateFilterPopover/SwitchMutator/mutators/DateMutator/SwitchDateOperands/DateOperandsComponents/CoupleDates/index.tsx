...
>(({ operation, pickedVariables }) => {
  ...
  return {
    ...
    pickedVariables:
      pickedVariables && pickedVariables instanceof Array ? pickedVariables : [null, null],
  };
}, CDMapDispatchToProps)(
  class extends React.Component<CoupleDateProps> implements ICoupleDateHandlers {
    public componentDidMount() {
      const { operation, setOperation, pickedVariables, setPickedVariables } = this.props;

      setOperation(operation);
      setPickedVariables(pickedVariables);
    }

    public handleOperandsChange: ViewProps['handleOperandsChange'] =
      (index) => (singleDateOperands) => {
        const { operation, operands, setOperation } = this.props;
        const newOperands: [SingleDateOperands, SingleDateOperands] = [operands[0], operands[1]];

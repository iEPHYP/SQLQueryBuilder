...
>(({ operation, tables, pickedVariables }, { column }) => {
  const stringOperation =
    operation.type === 'String' ? new StringOperation(operation) : new StringOperation();

  const { operator, caseSensitive } = stringOperation;
  const enums = getEnums(tables, column);
  const operands =
    !stringOperation.operands && enums && enums.length ? enums[0] : stringOperation.operands;
  ...
  class extends React.Component<StringMutatorProps> implements StringMutatorHandlers {
    ...
    public handleCaseSensitivityChange: ViewProps['handleCaseSensitivityChange'] = () => {
      ...
      setOperation(new StringOperation(operation, { caseSensitive: !caseSensitive }));
    };

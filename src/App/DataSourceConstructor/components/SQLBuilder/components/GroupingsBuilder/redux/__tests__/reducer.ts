...
describe('groupings reducer', () => {
  ...
  it('should add new grouping on ADD_GROUPING action', () => {
    expect(reducer([], addGrouping(new Grouping({ id: '1' })))).toMatchSnapshot();

    expect(
      reducer(
        [new Grouping({ id: '1' }), new Grouping({ id: '2' })],
        addGrouping(new Grouping({ id: '3' }))
      )
    ).toMatchSnapshot();
  });

  it('should update grouping on UPDATE_GROUPING action', () => {
    expect(reducer([], updateGrouping(new Grouping({ id: '4' })))).toMatchSnapshot();
  ...
  it('should remove grouping by id on REMOVE_GROUPING action', () => {
    ...
    expect(
      reducer([new Grouping({ id: '1' }), new Grouping({ id: '2' })], removeGrouping('4'))
    ).toMatchSnapshot();

    expect(
      reducer([new Grouping({ id: '4' }), new Grouping({ id: '2' })], removeGrouping('4'))
    ).toMatchSnapshot();
  });
  ...
  it('should clear all groupings on CLEAR_GROUPINGS action', () => {
    ...
    expect(reducer([new Grouping(), new Grouping()], clearGroupings())).toMatchSnapshot();
  });
});
